...
const ZaloButton: FC<HTMLAttributes<HTMLAnchorElement>> = ({ ...props }) => {
  ...
  useEffect(() => {
    ...
    if (isMobile) {
      linkzalo.current = `https://zalo.me/${import.meta.env.VITE_ZALO_PHONE_NUMBER}`;
    }
  }, []);
  ...
  return (
    <a href={linkzalo.current} target="_blank">
      <div className={cn("fixed z-20", props.className)}>
        <div className="fcta-zalo-nen-nut bg-blue-600 text-white size-12 md_size-14 rounded-full">
