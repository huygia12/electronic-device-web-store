...
import { CartItem, District, Product, Province, Ward } from "@/declare";
...
const CartCheckout = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const provincesRes = await axios.get<ProvinceRes>(
      ...
      Promise.all(promises)
        ...
        .catch((error) => {
          if (axios.isAxiosError(error)) {
            ...
            log.error("Axios error:", error.message);
            if (error.response) {
              log.error("Fetching shippingfee error: ", error.response.data);
              log.error("Response status: ", error.response.status);
            }
          } else {
            // General error handling
